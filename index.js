...
io.on("connect", (socket) => {
    ...
    socket.on("addView", async(data) => {
        ...
        if (liveUser) {
            ...
            if (joinedUserExist) {
                await LiveUser.updateOne({ _id: liveUser._id, "view.userId": data.userId }, {
                    $set: {
                        "view.$.userId": data.userId,
    ...
    socket.on("lessView", async(data) => {
        ...
        await LiveUser.updateOne({ _id: data.liveUserMongoId, "view.userId": data.userId }, {
            $set: {
                "view.$.isAdd": false,
            },
        });
    ...
    socket.on("addRequested", async(data_) => {
        ...
        if (liveUser) {
            ...
            if (joinedUserExist) {
                await LiveUser.updateOne({ _id: liveUser._id, "seat.userId": data.userId }, {
                    $set: {
                        "seat.$.userId": null,
                        "seat.$.image": null,
                        "seat.$.name": null,
            ...
            await LiveUser.updateOne({ _id: liveUser._id, "seat.position": data.position }, {
                $set: {
                    "seat.$.userId": data.userId,
                    "seat.$.image": null,
                    "seat.$.name": null,
    ...
    socket.on("declineInvite", async(data) => {
        const liveUser = await LiveUser.findOneAndUpdate({ _id: data.liveUserMongoId, "seat.position": data.position }, {
            $set: {
                "seat.$.userId": null,
                "seat.$.image": null,
                "seat.$.name": null,
